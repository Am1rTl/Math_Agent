...
#DEFAULT_MODEL = "openai/gpt-5-mini" # BEST NOW
				...
				while attempts < max_attempts and not step_completed:
					...
					if thought_action.action:
						...
						if action_type == 'FINISH':
							...
							print("   Агент предложил финальный ответ. Запуск верификации...")
							...
							if verification_result.get('decision') == 'accept':
								...
								print(f"   Финальный ответ ПОДТВЕРЖДЕН: {verification_result.get('summary', 'OK')}")
								...
								print(f"   Финальный ответ ОТКЛОНЕН: {verification_result.get('reason', 'N/A')}")
								new_steps = verification_result.get('new_steps', [])
								if new_steps:
									print(f"   Добавление {len(new_steps)} новых шагов в план...")
