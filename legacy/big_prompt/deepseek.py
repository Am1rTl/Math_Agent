		...
		try:
			...
			if usage:
				if isinstance(usage, dict):
					...
					total_tokens = getattr(usage, "total_tokens", None) or getattr(usage, "totalTokens", None) or 0
				agent_metrics.token_usage += total_tokens or 0
...
4. ЗАВЕРШЕНИЕ: Когда задача решена и ты получил целевые значения (a, вероятность, корни и т.п.), вызывай `FINISH` строго по схеме.
5. ГЛАВНОЕ ПРАВИЛО: Финальные числовые выводы должны появляться либо через инструмент, либо через чёткую аналитическую цепочку в Thought и `FINISH`. Если можно корректно завершить рассуждением (без тяжёлых вычислений), это разрешено.
6. ПРОМЕЖУТОЧНЫЕ ШАГИ: Используй `CALL_PYTHON`, только если вычисление нельзя надёжно выполнить вручную. Для чисто логических шагов достаточно Thought/ADD_STEP.
7. FINISH ДОЛЖЕН содержать trace_id из контекста и объект `values` с числовыми данными по цели задачи (число или список чисел).
...
Всегда проверяй, что финальный ответ содержит числовые значения в `values` и соответствует цели задачи.
Не добавляй никакого сопроводительного текста, '```json' или '```'.
